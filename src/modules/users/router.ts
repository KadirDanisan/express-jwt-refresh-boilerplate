...
import { userDestroyRequestSchema, userIndexRequestSchema, userShowRequestSchema, userUpdateRequestSchema, userCommentsRequestSchema } from './request-schemas';

const app = Router();

app.get('/', validateRequest(userIndexRequestSchema), UsersController.index);
app.get('/:id/comments', validateRequest(userCommentsRequestSchema), UsersController.getUserComments);
